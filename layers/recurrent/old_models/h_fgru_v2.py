...
class hGRU(object):
    ...
    def __init__(
        ...
        assert len(self.hgru_ids) == len(self.recurrent_k)
    ...
    def defaults(self):
        ...
        return {
            ...
            'readout': 'fb',  # l2 or fb
            'hgru_ids': ['h1', 'h2'],  # Labels for the hGRUs
    ...
    def prepare_tensors(self):
        ...
        if self.include_pooling:
            ...
            up_filters = self.recurrent_k[:2] + self.intermediate_ff
        ...
        # Create special case 2nd layer hgru FF drive
        setattr(
            self,
            'hgru_2_ff_kernel',
            tf.get_variable(
                name='hgru_2_ff_kernel',
                dtype=self.dtype,
                initializer=initialization.xavier_initializer(
                    shape=[1, 1] + [self.recurrent_k[0], self.recurrent_k[1]],
                    uniform=self.normal_initializer),
                trainable=True))
        setattr(
            self,
            ...
            tf.get_variable(
                name='hgru_2_ff_bias',
                dtype=self.dtype,
                initializer=tf.ones([self.recurrent_k[1]]),
                trainable=True))        
        ...
        setattr(
            ...
            'ff_cat_kernel_2',
            tf.get_variable(
                name='ff_cat_kernel_2',
                dtype=self.dtype,
                initializer=initialization.xavier_initializer(
                    shape=[1, 1] + [self.recurrent_k[1] * 2, self.recurrent_k[1]],
                    uniform=self.normal_initializer),
                trainable=True))
        setattr(
            self,
        ...
        setattr(
            ...
            'ff_cat_kernel_1',
            tf.get_variable(
                name='ff_cat_kernel_1',
                dtype=self.dtype,
                initializer=initialization.xavier_initializer(
                    shape=[1, 1] + [self.recurrent_k[0] * 2, self.recurrent_k[0]],
                    uniform=self.normal_initializer),
                trainable=True))
        setattr(
            self,
            ...
            tf.get_variable(
                name='ff_cat_bias_1',
                dtype=self.dtype,
                initializer=tf.ones([self.recurrent_k[0]]),
                trainable=True))

        # Create conv filters that supply top-drive
        prev_filters = self.recurrent_k[1]
        ...
        for idx, layer in enumerate(self.hgru_ids):
            ...
            self.m_shape = [
                ...
                self.recurrent_k[idx],
                self.recurrent_k[idx]]
            self.bias_shape = [1, 1, 1, self.recurrent_k[idx]]
            ...
            with tf.variable_scope(
                ...
                setattr(
                    ...
                    tf.get_variable(
                        ...
                        dtype=self.dtype,
                        initializer=initialization.xavier_initializer(
                            shape=self.h_ext[idx] + [self.recurrent_k[idx], self.recurrent_k[idx]],
                            uniform=self.normal_initializer),
                        trainable=True))
                setattr(
                    self,
    ...
    def hgru_ops(self, i0, x, h2, layer):
        """hGRU body."""
        layer_idx = self.hgru_ids.index(layer)
    ...
    def full(self, i0, x, l1_h2, l2_h2, fb_act_2, fb_act_1):
        ...
        l2_ff = tf.nn.conv2d(
            ...
            padding=self.padding)
        l2_ff = tf.nn.bias_add(
            l2_ff,
            getattr(self, 'hgru_2_ff_bias'))
        ...
        ff_drive = tf.nn.conv2d(
            input=ff_drive,
            filter=getattr(self, 'ff_cat_kernel_2'),
            strides=self.strides,
            padding=self.padding)
        fb_act_2 = tf.nn.bias_add(
            ff_drive,
            getattr(self, 'ff_cat_bias_2'))
        ...
        ff_drive = tf.nn.conv2d(
            ...
            strides=self.strides,
            padding=self.padding)
        fb_act_1 = tf.nn.bias_add(
            ff_drive,
            getattr(self, 'ff_cat_bias_1'))
    ...
    def build(self, x):
        ...
        l2_shape = tf.stack(
            [
                x_shape[0],
                x_shape[1] / self.pool_kernel[0],
                x_shape[2] / self.pool_kernel[0],
                self.recurrent_k[1]])
        ...
        if self.while_loop:
            ...
            # Prepare output
            i0, x, l1_h2, l2_h2, fb_act_2, fb_act_1 = returned

        else:
            i0 = 0
            for idx in range(self.timesteps):
                i0, x, l1_h2, l2_h2, fb_act_2, fb_act_1 = self.full(
                    i0=i0,
                    x=x,
                    l1_h2=l1_h2,
                    l2_h2=l2_h2,
