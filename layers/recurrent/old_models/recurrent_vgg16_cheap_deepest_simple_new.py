...
class Vgg16(GN, CreateGNParams, GNRnOps, GNFFOps):
    def __init__(
        ...
        if self.force_alpha_divisive:
            ...
            self.alpha_initializer = tf.constant_initializer(0.1)  # tf.zeros_initializer
        self.mu_initializer = tf.constant_initializer(0.)
        self.omega_initializer = tf.constant_initializer(0.1)  # tf.zeros_initializer
        self.kappa_initializer = tf.constant_initializer(0.)  # tf.zeros_initializer
    ...
    def build(self, i0, extra_convs=True):
        ...
        with tf.variable_scope('fgru'):
            error, fgru_activity = self.fgru_ops(  # h^(1), h^(2)
                ...
                layer_id=2,
                i0=i0)
        self.fgru_2 = fgru_activity  #  + self.conv4_3
        ...
        with tf.variable_scope('fgru'):
            error, fgru_activity = self.fgru_ops(  # h^(1), h^(2)
                ...
                layer_id=3,
                i0=i0)
        self.fgru_3 = fgru_activity  # self.conv5_3  #  + fgru_activity  # + self.conv5_3
