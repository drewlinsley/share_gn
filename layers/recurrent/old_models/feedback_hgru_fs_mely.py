...
class hGRU(object):
        ...
        for idx, layer in enumerate(self.hgru_ids):
            with tf.variable_scope(
                ...
                if self.gate_bias_init == 'chronos':
                    ...
                    bias_init_a = tf.ones(self.bias_shape)
                    bias_init_b = tf.ones(self.bias_shape)
                setattr(
                    self,
                    'gain_bias_%s_a' % layer,
                    tf.get_variable(
                        name='%s_gain_a_bias' % self.layer_name,
                        dtype=self.dtype,
                        trainable=True,
                        initializer=bias_init_a))
                setattr(
                    self,
                    'gain_bias_%s_b' % layer,
                    tf.get_variable(
                        name='%s_gain_b_bias' % self.layer_name,
                        dtype=self.dtype,
                        trainable=True,
                        initializer=bias_init_b))
                if self.gate_bias_init == 'chronos':
                    bias_init = -bias_init_a
                else:
                ...
                if self.alpha and not self.lesion_alpha:
                    setattr(
                        ...
                        'alpha_%s_a' % layer,
                        tf.get_variable(
                            name='%s_alpha_a' % self.layer_name,
                            initializer=initialization.xavier_initializer(
                    ...
                    setattr(
                        ...
                        'alpha_%s_b' % layer,
                        tf.get_variable(
                            name='%s_alpha_b' % self.layer_name,
                            initializer=initialization.xavier_initializer(
                                shape=self.bias_shape,
                                uniform=self.normal_initializer,
                                mask=None)))
    ...
    def circuit_input(self, h2, layer, var_scope, layer_idx):
        ...
        gain_kernels_a = getattr(self, 'gain_kernels_%s_a' % layer)
        gain_kernels_b = getattr(self, 'gain_kernels_%s_b' % layer)
        gain_bias_a = getattr(self, 'gain_bias_%s_a' % layer)
        gain_bias_b = getattr(self, 'gain_bias_%s_b' % layer)
        ...
        g1_a = self.gate_nl(g1_intermediate_a)
        ...
        g1_intermediate_b = self.conv_2d_op(
            data=h2,
            weights=gain_kernels_b,
            symmetric_weights=self.symmetric_gate_weights,
            dilations=self.dilations[layer_idx])
        with tf.variable_scope(
                '%s/g1_bn_b' % var_scope,
                reuse=self.scope_reuse) as scope:
            g1_intermediate_b = tf.contrib.layers.batch_norm(
                inputs=g1_intermediate_b + gain_bias_b,
                scale=True,
                center=False,
                fused=True,
                renorm=False,
                ...
                reuse=self.reuse,
                is_training=self.train)
        g1_b = self.gate_nl(g1_intermediate_b)
        ...
        with tf.variable_scope(
                '%s/c1_bn_a' % var_scope,
                reuse=self.scope_reuse) as scope:
            c1_a = tf.contrib.layers.batch_norm(
                inputs=c1_a,
                scale=True,
                center=False,
                fused=True,
        ...
        with tf.variable_scope(
                '%s/c1_bn_b' % var_scope,
                reuse=self.scope_reuse) as scope:
            c1_b = tf.contrib.layers.batch_norm(
                inputs=c1_b,
                scale=True,
                center=False,
                fused=True,
    ...
    def input_integration(self, x, c1_a, c1_b, h1, layer):
        """Integration on the input."""
        alpha_a = getattr(self, 'alpha_%s_a' % layer)
        mu_a = getattr(self, 'mu_%s_a' % layer)
        alpha_b = getattr(self, 'alpha_%s_b' % layer)
        mu_b = getattr(self, 'mu_%s_b' % layer)
        dynamics_a = (alpha_a * h1 + mu_a) * c1_a
        dynamics_b = (alpha_b * h1 + mu_b) * c1_b
        return self.recurrent_nl(x - (dynamics_a + dynamics_b))
    ...
    def hgru_ops(self, i0, x, h1, h2, layer, layer_idx):
        """hGRU body."""
        var_scope = '%s_hgru_weights' % layer
        # Circuit input receives recurrent output h2
        c1_a, c1_b = self.circuit_input(
            h2=h2,
            layer=layer,
            var_scope=var_scope,
            layer_idx=layer_idx)
    ...
    def condition(self, i0, x, l1_h1, l1_h2, l2_h1, l2_h2, fb_act):
    ...
    def build(self, x):
        ...
        # Prepare output
        i0, x, l1_h1, l1_h2, l2_h1, l2_h2, fb_act = returned
        if self.readout == 'l2':
            return l2_h2
