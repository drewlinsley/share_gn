...
class Vgg16(GN, CreateGNParams, GNRnOps, GNFFOps):
    def __init__(
        ...
        if self.force_alpha_divisive:
            ...
            self.alpha_initializer = tf.zeros_initializer
        self.mu_initializer = tf.zeros_initializer
        self.omega_initializer = tf.zeros_initializer
    ...
    def create_bn_params(self, constructor):
        ...
        for k, v in cons.iteritems():
            ...
            gamma = tf.get_variable(
                name='gamma_1x1_%s' % var_id,
                shape=[1, 1, 1, features],
                dtype=self.dtype,
                trainable=self.train,
                initializer=tf.constant_initializer(0.1))
            beta = tf.get_variable(
                name='beta_1x1_%s' % var_id,
                shape=[1, 1, 1, features],
                dtype=self.dtype,
                trainable=self.train,
                initializer=tf.constant_initializer(0.0))
            setattr(self, 'fgru_%s_td_gamma' % var_id, gamma) 
            setattr(self, 'fgru_%s_td_beta' % var_id, beta)
    ...
    def build(self, i0, extra_convs=False, conv_nl=tf.nn.relu):
        # Convert RGB to BGR
        ...
        with tf.variable_scope('fgru'):
            fgru_2_td = normalization.apply_normalization(
                ...
                trainable=self.train,
                reuse=self.reuse)
            fgru_2_td = self.fgru_2_td_gamma * fgru_2_td + self.fgru_2_td_beta
        ...
        with tf.variable_scope('fgru'):
            fgru_1_td = normalization.apply_normalization(
                ...
                trainable=self.train,
                reuse=self.reuse)
            fgru_1_td = self.fgru_1_td_gamma * fgru_1_td + self.fgru_1_td_beta
        ...
        with tf.variable_scope('fgru'):
            fgru_0_td = normalization.apply_normalization(
                ...
                trainable=self.train,
                reuse=self.reuse)
            fgru_0_td = self.fgru_0_td_gamma * fgru_0_td + self.fgru_0_td_beta
