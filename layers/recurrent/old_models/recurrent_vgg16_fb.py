...
class Vgg16(GN, CreateGNParams, GNRnOps, GNFFOps):
    ...
    def build(self, i0):
        ...
        # Resize and conv
        fgru_1_td = tf.image.resize_nearest_neighbor(
            self.fgru_2,
            self.fgru_1.get_shape().as_list()[1:3], align_corners=True)
        fgru_1_td = self.conv_layer(
            fgru_1_td,
            '5_to_3',
            learned=True,
            shape=[1, 1, fgru_1_td.get_shape().as_list()[-1], 256])
        error, fgru_activity = self.fgru_ops(  # h^(1), h^(2)
        ...
        fgru_0_td = tf.image.resize_nearest_neighbor(
            self.fgru_1,
        ...
        fgru_0_td = self.conv_layer(
            ...
            '3_to_1',
            learned=True,
            shape=[1, 1, fgru_0_td.get_shape().as_list()[-1], 64])
        error, fgru_activity = self.fgru_ops(  # h^(1), h^(2)
