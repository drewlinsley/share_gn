...
class Vgg16(GN, CreateGNParams, GNRnOps, GNFFOps):
    def __init__(
            ...
            recurrent_ff=False,
            strides=[1, 1, 1, 1],
            pool_strides=[4, 4],  # Because fgrus are every other down-layer
            pool_kernel=[4, 4],
            data_format='NHWC',
    ...
    def build(self, i0):
        ...
        # Resize and conv
        fgru_0_td_a = tf.image.resize_nearest_neighbor(self.fgru_2, self.fgru_0.get_shape().as_list()[1:3], align_corners=True)
        fgru_0_td_b = tf.image.resize_nearest_neighbor(self.fgru_1, self.fgru_0.get_shape().as_list()[1:3], align_corners=True)
        fgru_0_td = tf.concat([fgru_0_td_a, fgru_0_td_b], -1)
        fgru_0_td = self.conv_layer(fgru_0_td, '5_to_1', learned=True, shape=[1, 1, fgru_0_td.get_shape().as_list()[-1], 64]) 
